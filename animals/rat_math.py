...
class Rat(Animal):
    def visualize(self, image: np.ndarray) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """
        Simulate a simple rat-vision rendering from an RGB image.
        """
        ...
        animal_matrix = collapse_LMS_matrix(0.05, 0.86)
        ...
        result_in_rgb = apply_s_cone_vertical_gain(result_in_rgb, s_top=1.3, s_bottom=0.5, power=1.4, extra_boost=0.25)
