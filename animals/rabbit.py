...
class Rabbit(Animal):
    def visualize(self, image: np.ndarray) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """
        Simulate a simple rabbit-vision rendering from an RGB image.
        """
        ...
        animal_matrix = collapse_LMS_matrix(0.20, 1.01)
        result_in_rgb = vector_image_srgb @ animal_matrix.T
        result_in_rgb = result_in_rgb.reshape(linear_normalized_image.shape)

        # ---------- 5) apply blur ----------
        result_in_rgb = apply_anisotropic_acuity_blur_with_streak(result_in_rgb, 0.52, 0.9, 2.5, 5.0)
